...
echo "$($PSQL "SELECT name FROM teams RIGHT JOIN (SELECT DISTINCT(winner_id) FROM games) AS winners ON teams.team_id = winners.winner_id ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, name FROM teams RIGHT JOIN (SELECT winner_id, year FROM games WHERE round = 'Final') AS t ON teams.team_id = t.winner_id
ORDER BY year")"
